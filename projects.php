...
<?php
$sql = "SELECT * FROM projects WHERE students=''";
...
<th>Избери</th>
 ...
 <th>Проект</th>
    ...
    for ($i = 0; $i < $projects_cnt; $i++) {
            echo "
<form method='post' action='edit_project.php'><tr>
<td> <input type='radio' id='asd' name='projectId' value='" . $projects[$i]['id'] . "' required></td><td>" . $projects[$i]['title'] . "</td><td>" . $projects[$i]['description'] . "</td><td>" . $projects[$i]['students']
                . "";
    ...
    if($_SESSION['is_student']){
        ...
        echo "<input type='submit' name='new_project' value='Добави редактиран'><input type='submit' name='comment' value='Напиши коментар'><input type='submit' name='comments' value='Виж коментарите'></form></div>";
    }
